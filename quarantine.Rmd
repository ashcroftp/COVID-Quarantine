...
The infectivity curves are a schematic representation of the generation time distribution shown in Figure \\ref{fig:distributions}."
...
Quarantine efficacy is defined as the expected fraction of transmission that is prevented by quarantine across false-negative and positive testing individuals, as defined in Eq. \eqref{eq:Ftest} in Materials and methods.
...
By comparing two different standard quarantine strategies through their relative utility (i.e. the ratio of the utilities), we can eliminate the dependence on the fraction of infecteds (see Materials and methods).
...
This increase is only possible if the adherence level to the 10 day strategy is less than one third, as the maximum adherence cannot exceed 100\%.
The required increase in adherence grows rapidly as quarantine is shortened and soon becomes infeasible.
Hence the argument of shortening quarantine to increase adherence is of limited use.
Shortening to 7 days may be effective provided that adherence can increase by 30\% (relative adherence 1.30 [CI: 1.08,1.55]).
Changing to the test-and-release strategy, however, shows that the efficacy of the 10 day quarantine can be matched with release on day five or six with similar increases in adherence.
...
getFraction <- function(params) {
  lapply(a.vals, function(a) {
    ...
    fraction.symptomatic <- ifelse(tR.vals >= tQ,
                              getIntegral(upper = pmax(pmin(tR.vals, tS),0), lower = tQ, tE = tE, params = params),
                              NA)
    data.frame(
      tE = tE,
      tQ = tQ,
...
We use a symptom onset time of $t_E+5.7$, where 5.7 days is the mean incubation time (see Figure \\ref{fig:distributions}D).
The curve for $100\\%$ asymptomatic cases corresponds to the black curve in Figure \\ref{fig:contacts}A.
As in Figure \\ref{fig:contacts}, we fix the time of exposure at $t_E=0$ and the time of entering quarantine at $t_Q=3$.
Error bars reflect the uncertainty in the generation time distribution."
...
We can remove the symptomatic cases from the infectious pool as we assume that they would be isolated anyway, so this symptomatic transmission should not contribute to quarantine efficacy.
We assume self-isolation occurs immediately after symptom onset, but any delay here would mean more transmission is prevented by quarantine (Figure \ref{fig:contacts-asymptomatic-delay}).
...
\begin{equation}
F_{\rm test}(t_E,t_Q,t_T,t_R) = f(t_T-t_E) \int_{t_Q}^{t_R} {\rm d}t \, q(t-t_E) + \bigl[1-f(t_T-t_E)\bigr] \int_{t_Q}^{t_{\rm end}} {\rm d}t \, q(t-t_E),
...
\begin{equation}
\begin{aligned}
D &= (1-s)(t_R-t_Q) + s\left[f(t_T-t_E)(t_R-t_Q) + \bigl[1-f(t_T-t_E)\bigr](t_{\rm end}-t_Q)\right] \\
...
$D$ & Realised average duration of quarantine & $D=t_R-t_Q$ for standard quarantine \\ \hline
...
Concretely, for an efficacy $F$ [as defined by Eqs. \eqref{eq:F} or \eqref{eq:Ftest}], fraction of individuals in quarantine that are infected $s$, and average time spent in quarantine $D$, we define the utility as
\begin{equation}
U(s,F,D) = \frac{sF}{D}.
\label{eq:utility}
\end{equation}
...
Note that $F$ and $F^*$ are independent of $s$, but the duration of quarantine is dependent on $s$ for test-and-release strategies.
...
Note that the actual time spent in quarantine is $D = t_R-t_Q$ days, which is typically shorter than 10 days due to the delay between exposure and the start of quarantine.
...
\begin{equation}
\overline{F}_{\rm total}(y,t_R) = \frac{1}{y+1}\sum_{t_E=-y}^0 \int_0^{t_R} {\rm d}t \, q(t-t_E),
\label{eq:FtotalAve}
\end{equation}
...
\begin{equation}
F_{\rm local}(t_E,t_R) = \dfrac{\int_0^{t_R} {\rm d}t \, q(t-t_E)}{\int_0^\infty {\rm d}t \, q(t-t_E)},
\label{eq:Flocal}
\end{equation}
where we have again used $t_Q=0$.
Taking the average over the possible exposure times $-y \le t_E \le 0$ we have
\begin{equation}
\overline{F}_{\rm local}(y,t_R) = \frac{1}{y+1}\sum_{t_E=-y}^0 F_{\rm local}(t_E,t_R).
\label{eq:FlocalAve}
\end{equation}
...
\begin{equation}
F_{\rm reduced}(t_E,t_Q,t_R) = F(t_E,t_Q,t_R) + r \int_{t_R}^{t_{\rm end}} {\rm d}t \, q(t-t_E),
\label{eq:Freduced1}
\end{equation}
for the standard quarantine protocol, and
\begin{equation}
F_{\rm reduced}(t_E,t_Q,t_T,t_R) = F_{\rm test}(t_E,t_Q,t_T,t_R) + r f(t_T-t_E) \int_{t_R}^{t_{\rm end}} {\rm d}t \, q(t-t_E),
\label{eq:Freduced2}
\end{equation}
...
The average fraction of transmission prevented by (standard) quarantine is then $s \alpha(t_R-t_Q) F(t_E,t_Q,t_R)$, where $s$ is the fraction of individuals in quarantine that are infected, which we assume is independent of quarantine duration, and we have used $D=t_R-t_Q$.
...
\begin{equation}
\begin{aligned}
s \alpha(t_R-t_Q) F(t_E,t_Q,t_R) &= s \alpha(t_R^*-t_Q) F(t_E,t_Q,t_R^*)\\
\Rightarrow \quad \frac{\alpha(t_R-t_Q)}{\alpha(t_R^*-t_Q)} &= \frac{F(t_E,t_Q,t_R^*)}{F(t_E,t_Q,t_R)}.
\end{aligned}
\end{equation}
...
\begin{equation}
{\rm RA}(t_E,t_Q,t_R,t_R^*) = \frac{F(t_E,t_Q,t_R^*)}{F(t_E,t_Q,t_R)}.
\label{eq:adherence}
\end{equation}
...
\begin{equation}
{\rm RA}(t_E,t_Q,t_T,t_R,t_R^*) = \frac{F(t_E,t_Q,t_R^*)}{F_{\rm test}(t_E,t_Q,t_T,t_R)}.
\end{equation}
...
\begin{equation}
F_{\rm asymp}(t_E,t_Q,t_R,a,\Delta) = a \int_{t_Q}^{t_R} {\rm d}t \, q(t-t_E) +
(1-a) \int_{t_Q}^{\min(t_R,t_S+\Delta)} {\rm d}t \, q(t-t_E).
...
We use a symptom onset time of $t_E+5.7$, where 5.7 days is the mean incubation time (see Figure \\ref{fig:distributions}D).
As in Figures \\ref{fig:contacts} and \\ref{fig:contacts-further}, we fix the time of exposure at $t_E=0$ and the time of entering quarantine at $t_Q=3$.
Error bars reflect the uncertainty in the generation time distribution."
  ...
  lapply(delay.vals, function(delay) {
    lapply(a.vals, function(a) {
      fraction.asymptomatic <- ifelse(tR.vals >= tQ,
                                      ...
                                      NA)
      fraction.symptomatic <- ifelse(tR.vals >= tQ,
                                     getIntegral(upper = pmax(pmin(tR.vals, tS + delay),0), lower = tQ, tE = tE, params = params),
                                     NA)
      data.frame(
        tE = tE,
        tQ = tQ,
...
getFraction <- function(params) {
  ...
  lapply(a.vals, function(a) {
    lapply(tR.vals, function(tR) {
      ...
      fraction.symptomatic <- getIntegral(upper = pmax(pmin(tR, tS.vals),tQ), lower = tQ, tE = tE.vals, params = params)
      fraction <- mean(ifelse(maxPreventable < 1e-10, 1,
...
We assume that symptomatic individuals will immediately self-isolate at symptom onset.
We use a symptom onset time of $t_S=t_E+5.7$, where $5.7$ days is the mean incubation time (see Figure \\ref{fig:distributions}D).
